...
import { Database, GitBranch, Shield, ChevronRight, Layers, Zap } from "lucide-react";
...
// Cycle icons for design principles
const PRINCIPLE_ICONS = [GitBranch, Database, Shield, Layers, Zap];

function CaseStudyContent({ study }: { study: (typeof CASE_STUDIES)[0] }) {
  const contentRef = useRef(null);
  const contentInView = useInView(contentRef, { once: false, margin: "-60px" });
  ...
  return (
    ...
    >
      {/* Case study header */}
      <div className="bg-gradient-to-r from-primary/10 to-cyan/10 border-b border-border p-6 md:p-8">
        <div className="flex items-center gap-2 mb-3">
          <span className="tag tag-primary text-[10px]">Case Study</span>
          <span className="tag tag-cyan text-[10px]">Production</span>
        </div>
        <h3 className="text-2xl md:text-3xl font-bold text-text-primary mb-2">{study.title}</h3>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5 mb-3">
                    ...
                    <h5 className="font-semibold text-text-primary text-sm">{principle.title}</h5>
                  </div>
    ...
    <section id="system-design" className="section-padding relative overflow-hidden">
      ...
      <div className="container-width relative">
        ...
        <motion.div
          ...
          className="flex flex-wrap gap-3 justify-center mb-10 max-w-5xl mx-auto"
        >
          {CASE_STUDIES.map((study, i) => (
            <button
              key={study.id}
              onClick={() => setActiveIndex(i)}
              className={`relative px-5 py-3 rounded-xl text-sm font-medium transition-all duration-200 border text-left max-w-xs ${
                activeIndex === i
            ...
            >
              {activeIndex === i && (
                <motion.span
                  layoutId="tab-active"
                  className="absolute inset-0 bg-primary/10 rounded-xl border border-primary/40"
              ...
              <span className="relative z-10 flex items-start gap-2.5">
                ...
                <span className="leading-snug">{study.title}</span>
              </span>
            </button>
          ))}
        </motion.div>
        ...
        <div className="max-w-5xl mx-auto">
          <AnimatePresence mode="wait">
            <CaseStudyContent key={activeIndex} study={CASE_STUDIES[activeIndex]} />
          </AnimatePresence>
        </div>
      </div>
    </section>
